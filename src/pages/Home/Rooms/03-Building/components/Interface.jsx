    ...
    <div className="interface-container" style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: 'none', zIndex: 10, overflow: 'hidden' }}>
      ...
      <AnimatePresence mode="wait">
        ...
        {!isStarted && (
          ...
          >
            <motion.h1 initial={{ y: -120, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 1.2, delay: 2.5 }} style={{ color: 'white', fontFamily: 'Inter, sans-serif', fontSize: 'clamp(2rem, 8vw, 4rem)', letterSpacing: '10px', margin: 0 }}>TRES SILLINGS</motion.h1>
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 0.4 }} transition={{ duration: 1, delay: 0.8 }} style={{ color: '#00f2ff', fontFamily: 'monospace', letterSpacing: '5px', marginTop: '10px', fontSize: '0.8rem' }}>Portfolio 1.0</motion.p>
            <motion.button onClick={onStart} initial={{ y: 60, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 1, delay: 2.7 }} whileHover={{ scale: 1.05, boxShadow: "0 0 30px rgba(0,242,255,0.4)", borderColor: "#fff", color: "#fff" }} whileTap={{ scale: 0.95 }} style={{ marginTop: '60px', padding: '15px 45px', background: 'transparent', border: '1px solid #00f2ff', color: '#00f2ff', fontFamily: 'Inter, sans-serif', fontWeight: 'bold', cursor: 'pointer', letterSpacing: '4px', textTransform: 'uppercase', transition: 'all 0.3s ease' }}>Enter Portfolio</motion.button>
          </motion.div>
        )}
